...
class obj_det_pipeline_model_yolov3(obj_det_evaluator, pipeline_model):
	...
	def load(self):
		# Load your model
		# self.model = torch.hub.load('ultralytics/yolov5', 'yolov5s')
		input_dir = r'data/obj_det/datasets/karthika95-pedestrian-detection/2022-06-23_14:08:31.125208'
		test_path=os.path.join(input_dir, 'Test/Test/JPEGImages')
		test_annot=os.path.join(input_dir, 'Test/Test/Annotations')
		xtest = self.generate_data(test_annot, test_path)
		#print(xtest)
		# pass
	...
	def predict(self, x: dict) -> pd.DataFrame:
		...
		#print(x)
		global df_temp,glob_count,tp,fn,fp,iou_list,iou_avg
		input_dir = r'data/obj_det/datasets/karthika95-pedestrian-detection/2022-06-23_14:08:31.125208'
		test_path=os.path.join(input_dir, 'Test/Test/JPEGImages')
		test_annot=os.path.join(input_dir, 'Test/Test/Annotations')
		xtest = self.generate_data(test_annot, test_path)
		...
		def iou_mapping(box_yolo,gt_boxes):
			global df_temp,glob_count,tp,fn,fp,iou_list
			overall_iou = []
			max_iou = 0
			for i in gt_boxes:
			...
			if max_iou == 0:
				...
				if max_iou > 0.5:
					tp += 1
					df_temp.loc [glob_count-1 , 'gt_x'] = gt[0]
					df_temp.loc [glob_count-1 , 'gt_y'] = gt[1]
					df_temp.loc [glob_count-1 , 'gt_xmax'] = gt[2]
					df_temp.loc [glob_count-1 , 'gt_ymax'] = gt[3]
					df_temp.loc [glob_count-1 , 'iou'] = max_iou
				else:
					fp += 1
		...
		def detector(image, num,gt_boxes):
			...
			for i in pindex:
				...
				if person_class_ids[i]==0:
					...
					df_temp = df_temp.append(dict_csv,ignore_index = True)
		...
		for i in paths:
			...
			detector(img, it, gt_boxes)
			df_temp.to_csv('output.csv',index = False)
			it += 1
		predict_results = df_temp
		iou_avg = sum(iou_list) / len(iou_list)
		return {tp,fn,fp,iou_avg}, predict_results
		pass
	...
	def evaluate(self, x, y):
		...
		global df_temp,glob_count,tp,fn,fp,iou_list,iou_avg
		RESULTS, PREDICTIONS = {tp,fn,fp,iou_avg}, self.predict(x)
		#df_temp = pd.read_csv('output.csv')
		# print(x)
		# print(y)
		prec = tp / float(tp + fp)
		...
		f1_score = 2*prec*recall/(prec+recall)
		iou_avg = sum(iou_list) / len(iou_list)
		RESULTS = {'precision:':prec,'recall:':recall,'F1 Score:':f1_score,'Average IOU:':iou_avg}
